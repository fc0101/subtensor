...
// Implement SubtensorEpochReturnTypeTrait for Vec<(T::AccountId, u64, u64)>
impl<T: Config> SubtensorEpochReturnTypeTrait<T> for Vec<(T::AccountId, u64, u64)> {
    fn get(&self) -> Vec<I32F32> {
        self.encode().iter().map(|&x| I32F32::from_num(x)).collect()
    }
}

// Implement SubtensorEpochReturnTypeTrait for Vec<I32F32>
impl<T: Config> SubtensorEpochReturnTypeTrait<T> for Vec<I32F32> {
    fn get(&self) -> Vec<I32F32> {
        self.clone()
    }
}
